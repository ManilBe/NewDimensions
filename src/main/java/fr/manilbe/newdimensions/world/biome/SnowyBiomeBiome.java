...
package fr.manilbe.newdimensions.world.biome;
...
public class SnowyBiomeBiome extends NewDimsElements.ModElement {
	@ObjectHolder("newdims:snowy_biome")
	public static final CustomBiome biome = null;
	public SnowyBiomeBiome(NewDimsElements instance) {
		super(instance, 1);
	}
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(Blocks.ACACIA_LOG.getDefaultState()),
									new SimpleBlockStateProvider(Blocks.DARK_OAK_LEAVES.getDefaultState()))).baseHeight(9)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		@Override
		protected boolean func_225557_a_(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.SNOW_BLOCK.getDefaultState().getBlock() || ground == Blocks.DIRT.getDefaultState().getBlock())
							&& (ground2 == Blocks.SNOW_BLOCK.getDefaultState().getBlock() || ground2 == Blocks.DIRT.getDefaultState().getBlock())))
						return false;
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.ACACIA_LOG.getDefaultState().getBlock()
					|| blockType == Blocks.DARK_OAK_LEAVES.getDefaultState().getBlock() || blockType == Blocks.SNOW_BLOCK.getDefaultState().getBlock()
